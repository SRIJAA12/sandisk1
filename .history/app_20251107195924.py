                ...
                if len(saved_frames) > 0:
                    try:
                        ...
                        for codec_name, ext in codecs_to_try:
                            try:
                                test_path = os.path.join(tmpdir, f"test_{codec_name}{ext}")
                                ...
                                for frame_idx, frame in enumerate(saved_frames):
                                    try:
                                ...
                                # Check if enough frames were written
                                if written > 0 and os.path.exists(test_path):
                                    ...
                                    if file_size > 1.0:  # At least 1MB
                                        ...
                                        st.success(f"✅ Video created ({codec_name}): {written}/{len(saved_frames)} frames | {file_size:.2f} MB")
                                        video_created = True
                                        break
                                    else:
                                        os.remove(test_path)
                        ...
                        if not video_created:
                            error_msg = "No codec could write frames successfully"
