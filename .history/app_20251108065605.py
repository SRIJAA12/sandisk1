...
with tab1:
    ...
    if video_file:
        ...
        if st.button("🚀 START ANALYSIS", type="primary", use_container_width=True):
           ...
           # ===================================================================
# VIDEO CREATION (UNIFIED)  <-- REPLACE BOTH EXISTING VIDEO CREATION BLOCKS WITH THIS
# ===================================================================

st.markdown("---")
st.markdown("### 🎬 Creating Optimized Video (AURA MP4, H.264 CRF18)...")

# Ensure output file placed inside the same tmpdir (storage: A)
...
if len(saved_frames) == 0:
    video_creation_status.error("❌ No frames to save! Video cannot be created.")
    video_created = False
else:
    # Show configuration
    st.markdown(f"""
    <div class="glass-card">
        <h4>Video Creation Configuration</h4>
        <p>📊 Frames to write: <strong>{len(saved_frames)}</strong></p>
        <p>🎞️ FPS: <strong>{fps:.1f}</strong></p>
        <p>📐 Resolution: <strong>{width}×{height}</strong></p>
        <p>⚙️ Codec: <strong>H.264 (mp4)</strong> | CRF: <strong>18</strong></p>
    </div>
    """, unsafe_allow_html=True)

    # Try to create video and show progress spinner while building
    progress_bar_video = st.progress(0, text="Starting encoding...")
    try:
        # Call video generator (ffmpeg preferred - configured to CRF18)
        ...
        progress_bar_video.progress(1.0, text="Encoding finished")
        ...
        progress_bar_video.progress(1.0, text="Encoding failed")
        video_creation_status.error(f"❌ Video creation exception: {str(e)}")
        video_created = False

# ===================================================================
# VIDEO DISPLAY (autoplay + loop + muted preferred for demo)
# ===================================================================
...
if video_created and os.path.exists(output_path):
    try:
        # Attempt HTML5 embed with autoplay + loop + muted (auto_loop option)
        import base64
        import streamlit.components.v1 as components

        # read bytes and base64 encode (be mindful of large files)
        ...
        <video
            ...
            muted
            playsinline
            style="width:100%; max-height:520px; border-radius:8px; box-shadow: 0 6px 20px rgba(0,0,0,0.35);"
        >
        ...
        # Embed via components (this will autoplay+loop+mute)
        components.html(video_html, height=520)

    except Exception as e_embed:
        # Fallback to streamlit's native video player (no autoplay)
        try:
            st.warning("Autoplay embed failed; using fallback player.")
            st.video(output_path)
        except Exception as e2:
            st.error("Cannot display video in browser.")
            try:
                ...
                **Storage Saved:** {storage_saved_gb:.2f} GB ({reduction:.1f}%)
                ...
                if video_created and os.path.exists(output_path):
                    try:
                        ...
                        st.warning("⚠️ Cannot display in browser")
