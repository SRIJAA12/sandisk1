...
with tab1:
    ...
    if video_file:
        ...
        if start_analysis_button:
            ...
            # Create output video with better codec
            output_video_path = os.path.join(tmpdir, "aura_optimized.avi")
            ...
            if len(saved_frames) > 0 and fps > 0:
                try:
                    ...
                    st.write(f"Video parameters: {width}×{height} @ {fps:.1f} FPS")
                    ...
                    if not out_writer.isOpened():
                        ...
                        if os.path.exists(output_video_path) and os.path.getsize(output_video_path) > 0:
                            ...
                            st.success(f"✅ Optimized video created: {frames_written} frames | {file_size_mb:.2f} MB")
                            video_created = True
                        else:
                ...
                st.warning("⚠️ No frames to save or invalid FPS")
